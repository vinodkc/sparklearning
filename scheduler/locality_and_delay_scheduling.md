...
> **Think of it like picking up a package.** If the package is already in your house (PROCESS_LOCAL), you just grab it. If it's at your neighbour's door (NODE_LOCAL), you walk next door. If it's across town (ANY), you drive. Given the choice, you'd rather the courier had left it on your doorstep. Data locality is that preference encoded into the scheduler.
...
> **It's like a set of concentric circles.** Process-local is right at the centre—you and the data are in the same room. Node-local is the same floor of the building. Rack-local is the same building. Any is anywhere in the city. The scheduler tries to pick the innermost circle it can before accepting anything further away.

When the stage's tasks are queued, they're grouped by these levels—by executor, by host, by rack, and "no preference." So for an offer from executor E on host H, the scheduler can quickly see: "Do I have a task that wants this executor? This host? This rack? Or only a task with no preference?" It tries in that order and picks the first match that's *allowed* by the current **delay scheduling** rule.
...
> **It's like waiting for a direct flight instead of taking a connecting one.** The direct flight leaves in 2 hours; a connecting flight leaves now but adds 4 hours of total travel. You might choose to wait at the gate for the direct flight rather than board immediately and pay a bigger cost later. Delay scheduling is that calculation applied to task placement.
