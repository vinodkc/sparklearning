...
> **FIFO is like a single checkout lane at a supermarket.** The person who arrived first gets served first. If someone in front of you has 200 items, you wait until they're done—even if you only have one bottle of milk. Simple, predictable, but unfair when workloads are mixed.

That simplicity can backfire. One long or heavy job can starve others. If you're sharing an application—for example, many users in a single Thrift server—you usually want some form of **fair sharing** so that no single job or user monopolizes the cluster.
...
> **Think of scheduling pools like lanes at a bank.** There is a "priority" lane with a guaranteed number of teller windows reserved for business customers (minimum share), and a general lane for everyone else. If the priority lane is empty, those windows serve the general queue. Extra windows beyond the guaranteed ones are shared among all lanes in proportion to their lane weights. No single customer can hold all the windows indefinitely.

When a job is submitted, it is assigned to a **pool**. That assignment comes from a property set on the thread that submits the job—so different threads or users can set different pool names and their jobs go into different pools. If the named pool doesn't exist yet, Spark can create it with default settings. So you can have one pool per user, one per team, or one per priority level.

---

## How the fair ordering works

When the scheduler has to decide which task set gets the next chance at resource offers, it walks the **tree**: root → pools → task sets. At each level, it **sorts** the children using the pool's policy (FIFO or fair).
...
> **It's like dividing pizza at a party.** Each guest has a guaranteed slice (minimum share). Once everyone has their guaranteed slice, the remaining slices are divided by appetite (weight): hungrier guests (higher weight) get more of what's left. The hungriest guest who hasn't had their guaranteed slice eats first; after that, the guest with the least pizza relative to their appetite eats next.

Within a pool, if the mode is FIFO, stages are ordered by submission order. If the mode is FAIR, the same fair rule applies to the task sets inside that pool. So you get a consistent ordering from root down to the actual task sets.

---

## How jobs get into a pool

A job carries **properties** (e.g. a pool name, or a job group). Those properties are usually set on the **thread** that will submit the job—for example, "all jobs from this thread use pool *analytics*." So before running an action, the application sets the pool name for the current thread; when the job is submitted, the scheduler reads that property and adds the job's task sets to the corresponding pool. Pools can also be **predefined** in a config file (e.g. an XML file), with explicit minimum shares and weights per pool, so that important groups get guaranteed capacity.
